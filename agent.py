...
EXTRA_TOOLS: List[Dict[str, Any]] = [
    # next_available_day moved to the MCP server at step 2.2. The server owns the
    # name now: one owner, one schema, and the pod gets fare_rules with it.
...
def run_agent(pnr: str, last_name: str, message: str) -> str:            # ✏️ Build 1, step 1.2
    ...
    tools = tool_list()
    # Caching is a prefix match, so the stable text goes first and carries the
    # breakpoint; runtime_preamble()'s clock changes every second and would
    # invalidate everything after it if it stayed at the front.
    system = [
